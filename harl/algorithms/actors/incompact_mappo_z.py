...
class IncompatMAPPOZ:
    def __init__(self, config):
        ...
        self.opts = [
            optim.Adam(net.parameters(), lr=config.lr, eps=1e-5)
            for net in [self.policy_net, self.sp_critic, *self.xp_critics, self.discrim]
        ]
        assert (getattr(config, "mb_size", False) > 0) ^ (
            getattr(config, "num_mb", False) > 0
    ...
    def set_param(self, param):
        ...
        assert len(self.xp_critics) == len(
            param["xp_critic_param_list"]
        ), f'{len(self.xp_critics)}, {len(param["xp_critic_param_list"])}'
    ...
    def act(self, inp):
        ...
        if getattr(self.config, "get_act_dist", False):
            if isinstance(self.act_space, Discrete):
                ...
                return [
                    Arrdict(action=a, logprob=l, value=v, act_dist=dist)
    ...
    def to_cuda(self):
        for net in self.nets:
            net.to("cuda")
        self.device = self.training_device
    ...
    def compute_loss(self, minibatch, critic, minimize_reward=False, pg_coef=1.0):
        ...
        pg_loss2 = mb_adv * torch.clamp(
            ratio, 1 - self.config.clip_param, 1 + self.config.clip_param
        )
        ...
        v_clipped = minibatch.decision.value + torch.clamp(
            cur_values - minibatch.decision.value,
        ...
        if self.config.discrim_coef > 0:
            ...
            if isinstance(self.act_space, Discrete):
                ...
                pred_z = self.discrim(
                    minibatch.inp.data.obs,
                    torch.cat(
                        [dist.mean, dist.scale_tril.diagonal(dim1=1, dim2=2)], axis=1
                    ),
                )
            else:
                raise NotImplementedError
        ...
        loss = (
            (-1) ** minimize_reward * pg_coef * pg_loss
            - self.config.ent_coef * entropy_loss
            + v_loss * self.config.vf_coef
            + self.config.discrim_coef * discrim_loss
        )
        return loss
    ...
    def learn(self, batches, critics, pg_mask, value_mask):
        ...
        indices = [
            np.arange(batch.inp.data.obs.shape[0]) if batch is not None else [None]
            for batch in batches
        ]
        [np.random.shuffle(ind) for ind in indices]
        batch_size = max(
            [
                batch.inp.data.obs.shape[0] if batch is not None else 0
                for batch in batches
            ]
        )
        mb_size = getattr(self.config, "mb_size", None) or int(
            np.ceil(batch_size / self.config.num_mb)
        )

        for start in range(0, batch_size, mb_size):
            losses = []
            for i, (batch, critic, pm, vm) in enumerate(
                zip(batches, critics, pg_mask, value_mask)
            ):
                if batch is None:
                    continue
                if not (pm or vm):
            ...
            (
                ...
                for net in [self.policy_net, self.sp_critic, *self.xp_critics]
            )
            for opt in self.opts:
                opt.step()
    ...
    def train(
        ...
        **kwargs,
        ...
        pg_mask = np.concatenate([pg_mask, np.array([1.0])])
        value_mask = np.concatenate([value_mask, np.array([1.0])])

        assert (
            len(batches) == len(critics) == len(pg_mask) == len(value_mask)
        ), f"{len(batches)},{len(critics)},{len(pg_mask)},{len(value_mask)}"
...
class PolicyNet(nn.Module):
    def __init__(self, obs_space, action_space, config):
        ...
        if obs_space.__class__.__name__ == "list":
            obs_size = np.prod(np.array(obs_space))
        else:
            obs_size = (
                obs_space.n
        ...
        if isinstance(action_space, Discrete):
            ...
            self.logstd = nn.Parameter(
                torch.zeros(act_size) + np.log(config.pol_init_var)
            )
        else:
            raise NotImplementedError

        self.layers = FCLayers(
            obs_size + config.z_dim, hidden_size, num_hidden, act_size, head_std=0.01
        )
...
class CentralizedValueNet(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.joint_action_size = (
            self.config.act_space.n
            if isinstance(self.config.act_space, Discrete)
            else np.prod(self.config.act_space.shape)
        )
        hidden_size = getattr(config, "hidden_size", 64)
        num_hidden = getattr(config, "num_hidden", 2)
        self.use_cnn = False
        self.layers = FCLayers(
            config.state_shape[0], hidden_size, num_hidden, 1, head_std=1.0
        )
...
class Discriminator(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.obs_space.__class__.__name__ == "list":
            obs_size = np.prod(np.array(config.obs_space))
        else:
            obs_size = (
                config.obs_space.n
                if isinstance(config.obs_space, Discrete)
                else np.prod(config.obs_space.shape)
            )
        n_actions = (
            config.act_space.n
            if isinstance(config.act_space, Discrete)
            else np.prod(config.act_space.shape) * 2
        )
...
class FCLayers(nn.Module):
    def __init__(
        ...
        head_std=0.01,
        last_linear=True,
        layer_init_fn=ortho_layer_init,
    ):
        assert num_hidden > 0
        super().__init__()
        ...
        pre_hidden = [
            layer_init_fn(layer_construct(input_dim, hidden_size), base_std),
            activation_fn(),
        ]
        base = pre_hidden
        if num_hidden > 1:
            ...
            for i in range(num_hidden - 1):
                hiddens.append(
                    layer_init_fn(layer_construct(hidden_size, hidden_size), base_std)
                )
