...
def default_tools(f):
    ...
    f.js_on_event(
        boe.DoubleTap, bom.callbacks.CustomJS(args=dict(p=f), code="p.reset.emit()")
    )
...
def _timeseries(source, x, y):
    ...
    f = bop.figure(
        x_range=bom.DataRange1d(start=0, follow="end"), tooltips=[("", "$data_y")]
    )
...
class Stream:
    ...
    def _new_grid(self, children):
        return bol.gridplot(
            children, ncols=4, plot_width=350, plot_height=300, merge_tools=False
        )
