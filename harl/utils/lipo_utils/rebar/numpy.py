...
def infer_dtype(exemplar):
    return np.dtype(
        [
            (k, v.dtype if isinstance(v, np.generic) else type(v))
...
def make_header(dtype):
    ...
    assert (
        not dtype.hasobject
    ), "Arrays with objects in get pickled, so can't be appended to"
...
class Writer:
    ...
    def write_many(self, ds):
        for channel, d in ds.items():
            if channel not in self._writers:
                path = paths.path(self._run_name, self._group, channel).with_suffix(
                    ".npr"
                )
                self._writers[channel] = FileWriter(path)
            self._writers[channel].write(d)
...
def test_write_read():
    ...
    writer.write(
        "mean/traj-length", {"total": 65536, "count": 14, "_time": np.datetime64("now")}
    )
    writer.write(
        "max/reward", {"total": 50000.5, "count": 50, "_time": np.datetime64("now")}
    )
