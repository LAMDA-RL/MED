...
class FSM:
    ...
    def _reset(self, reset):
        if reset.any():
            n_reset = reset.sum()
            self._token[reset] = torch.distributions.Categorical(self._start).sample(
                (n_reset,)
            )
    ...
    def dataframe(self, **kwargs):
        ...
        successor = self._trans[
            torch.arange(self.n_states, device=self.device), soln.policy
        ].argmax(-1)
...
class State:
    ...
    def to(self, state, action=0, reward=0.0, weight=1.0):
        ...
        self._builder._trans.append(
            dotdict.dotdict(
                prev=self._name, action=action, next=state, reward=reward, weight=weight
            )
        )
        return self
...
def fsm(f):
    def init(self, n_envs=1, *args, **kwargs):
        ...
        assert isinstance(
            fsm, dict
        ), "FSM description must be a dictionary. Did you forget to call `.build()`?"
