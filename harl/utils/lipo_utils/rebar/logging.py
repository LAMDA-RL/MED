...
class IPythonRenderer:
    def __init__(self, compositor=None):
        ...
        self._buffers = defaultdict(
            lambda: deque([""] * self._out.lines, maxlen=self._out.lines)
        )
...
def to_dir(run_name):
    ...
    handler.setFormatter(
        logging.Formatter(
            fmt="%(asctime)s %(levelname)s %(name)s: %(message)s", datefmt=r"%H:%M:%S"
        )
    )
    ...
    with to_dir(run_name):
        try:
            ...
            thread = threading.Thread(
                target=_from_dir, args=(canceller, renderer, reader)
            )
            thread.start()
            yield
        finally:
