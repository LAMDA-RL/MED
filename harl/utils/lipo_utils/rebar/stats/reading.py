    ...
    def arrays(self):
        ...
        for channel, new in self._reader.read().items():
            ...
            if field.startswith(self._prefix):
                current = (
                    [self._arrs[category, field]]
                    if (category, field) in self._arrs
                    else []
                )
    ...
    while True:
        if time.time() > nxt:
            ...
            if len(values) > 0:
                ...
                content = "\n".join(
                    f"{{:{key_length}s}} {{}}".format(k, format(values[k]))
            ...
            out.refresh(
                f"{run_name}: {tdformat(age)} old, {rule} rule, {size:.0f}MB on disk\n\n{content}"
            )
    ...
    if logging.in_ipython():
        try:
            ...
            thread = threading.Thread(
                target=_from_dir, args=(canceller, run_name, out, rule)
            )
            thread.start()
            yield
        finally:
