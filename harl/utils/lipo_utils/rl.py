...
from harl.utils.lipo_utils import Arrdict, arrdict
from harl.utils.lipo_utils.nn import dict_to_tensor


def get_discount_coef(gamma, ep_len, device=None):
    t_prime = torch.arange(0, ep_len, device=device).unsqueeze(1)
    power = torch.abs(
        torch.arange(0, ep_len, device=device).unsqueeze(0).repeat(ep_len, 1) - t_prime
    )
    return gamma**power
...
def compute_jsd_metric(batch, agent_pop, kernel_gamma, use_br):
    ...
    for ep in chop_into_episodes(batch):
        ...
        for i in range(pop_size):
            ep.act_logprob[i] = calc_action_logprob(
                agent_pop[i], ep
            ).detach()  # [ep_len,]
            ep.pi[i] = torch.sum(ep.act_logprob[i], axis=0).exp().detach()
            ep.delta[i, :] = local_action_kernel(
                ep.act_logprob[i], ep, kernel_gamma
            ).detach()
...
def local_action_kernel(a_logprob, batch, gamma):
    ...
    delta = torch.sum(
        d * a_logprob.unsqueeze(0), axis=1
    ).exp()  # [ep_len] (delta[t]) is delta_t
    return delta
...
def chop_into_episodes(traj):
    ...
    if isinstance(traj.outcome.done, dict):
        ...
        assert (
            np.array(term_t) == term_t[0]
        ).all(), f"All agents must terminates at the same time, got {term_t}"
    # flatten traj
...
def compute_return(traj, gamma, next_value):
    ...
    for t in reversed(range(traj.outcome.reward.shape[0])):
        ...
        assert (
            ret is not None
        ), f"Please provided the next_value when the trajectory is not terminated"
        ret = rew[t] + gamma * ret
...
def test_compute_gae():
    traj = Arrdict(
        outcome=Arrdict(reward=np.array([1, 1, 0, 1]), done=np.array([0, 0, 0, 1]))
    )
    ...
    assert (
        abs(traj.outcome.adv - np.array([2.37535185, 0.443717, -1.713, -3])).sum()
        < 0.00001
    )

    traj = Arrdict(
        outcome=Arrdict(reward=np.array([1, 1, 0, 1]), done=np.array([0, 1, 0, 1]))
    )
    values = np.array([1, 2, 3, 4])
    next_value = np.array(10)
    compute_gae(traj, 0.99, 0.9, values, next_value)
    ...
    traj = Arrdict(
        outcome=Arrdict(reward=np.array([1, 1, 0, 1]), done=np.array([0, 1, 0, 0]))
    )
    values = np.array([1, 2, 3, 4])
    next_value = np.array(10)
    compute_gae(traj, 0.99, 0.9, values, next_value)
...
def test_compute_return():
    ...
    traj = Arrdict(
        outcome=Arrdict(reward=np.array([1, 1, 0, 1]), done=np.array([0, 0, 0, 0]))
    )
    compute_return(traj, 0.99, 0)
    assert (traj.outcome.ret - np.array([2.960299, 1.9801, 0.99, 1])).sum() < 1e-5
    # print(traj.outcome.ret)
    ...
    traj = Arrdict(
        outcome=Arrdict(
            reward=np.array([1, 2, 3, 1, 2, 3]), done=np.array([0, 0, 1, 0, 0, 0])
        )
    )
    ...
    assert (
        traj.outcome.ret - np.array(returns)
    ).sum() < 1e-5, f"{traj.outcome.ret} vs {returns}"
    # print(traj.outcome.ret)
