...
class SARDConsistencyChecker(Wrapper):  # sub-class MultiagentEnv
    ...
    def reset(self, seed=None):
        self.last_data = self.env.reset(seed=seed)
        return self.last_data

    def step(self, decision):
        assert (
            getattr(self.last_data, "obs", None) is not None
...
class ZWrapper(Wrapper):
    ...
    def __init__(
        self, env, z_dim, z_discrete, z_range=None, shared_z=True, *args, **kwargs
    ):
        super().__init__(env)
    ...
    def _resample_z(self):
        ...
        if isinstance(self.z_dim, int):
            if self.z_discrete:
                z = random.choices(
                    np.eye(self.z_dim, dtype=np.float32), k=len(self.players)
                )
            else:
                z = np.random.uniform(
                    *self.z_range, size=(len(self.players), self.z_dim)
                ).astype(np.float32)
            if self.shared_z:
            ...
            for p, z_dim in self.z_dim.items():
                if self.z_discrete:
                    ...
                    self.z[p] = np.random.uniform(
                        *self.z_range, size=(1, self.z_dim)
                    ).astype(np.float32)[0]
...
class AgentIDWrapper(Wrapper):
    def __init__(self, env, *args, **kwargs):
        assert np.array(
            [len(v["obs"].shape) == 1 for k, v in env.observation_spaces.items()]
        ).all()
        self.env = env
    ...
    def get_observation_space(self):
        ...
        new_obs_space = Dotdict(
            obs=spaces.Box(-1, 1, shape=(obs_space["obs"].shape[0] + self.num_players,))
        )
    ...
    def _add_id(self, data):
        for p in data.keys():
            ...
            data[p]["obs"] = np.concatenate(
                [data[p]["obs"], self.name_id_map[p]], axis=-1
            )
        return data

    def reset(self, seed=None):
        data = self.env.reset(seed)
...
class RescaleActionsWrapper(Wrapper):
    def __init__(self, env, min_action, max_action, *args, **kwargs):
        ...
        super().__init__(env)
        self.min_action = {
            p: np.zeros(env.action_spaces[p].shape, dtype=env.action_spaces[p].dtype)
            + min_action
            for p in env.players
        }
        self.max_action = {
    ...
    def step(self, decision):
        ...
        for p in out:
            ...
            action = low + (high - low) * (
                (action - self.min_action[p])
                / (self.max_action[p] - self.min_action[p])
            )
...
class ClipActionsWrapper(Wrapper):
    def step(self, decision):
        ...
        for p in out:
            action = out[p]["action"]
            out[p]["action"] = np.clip(
                action, self.action_spaces[p].low, self.action_spaces[p].high
            )
        return self.env.step(out)
...
class NormalizedWrapper(Wrapper):
    def __init__(
        ...
        **kwargs,
        ...
        self.ob_rms = (
            {
                p: RunningMeanStd(shape=self.get_observation_space().obs.shape)
                for p in env.players
            }
            if norm_obs
            else None
        ...
        if norm_state:
            self.state_rms = {
                p: RunningMeanStd(shape=self.get_state_shape()) for p in env.players
            }
        self.ret_rms = (
            {p: RunningMeanStd(shape=(1,)) for p in env.players} if norm_ret else None
        )
        self.clipob = clipob
    ...
    def _obfilt(self, obs, p):
        if self.state_rms and (p in self.state_rms):
            ...
            obs = np.clip(
                (obs - self.ob_rms[p].mean)
                / np.sqrt(self.ob_rms[p].var + self.epsilon),
                -self.clipob,
                self.clipob,
            ).astype(np.float32)
        return obs
    ...
    def _statefilt(self, state, p):
        if self.state_rms and (p in self.state_rms):
            ...
            state = np.clip(
                (state - self.state_rms[p].mean)
                / np.sqrt(self.state_rms[p].var + self.epsilon),
                -self.clipob,
                self.clipob,
            ).astype(np.float32)
        return state
