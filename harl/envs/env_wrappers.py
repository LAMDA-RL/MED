...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == "step":
            if data.get("entity_state", False):
                (
                    entity_masks,
                    entities,
                    reward,
                ...
                if "bool" in done.__class__.__name__:  # done is a bool
                    # TODO: remove this condition if tested no problem
                    assert False

                if np.all(
                    done
                ):  # if done, save the entity_masks, entities, and available actions in info, and then reset
                    info[0]["original_entity_masks"] = copy.deepcopy(entity_masks)
                    info[0]["original_entities"] = copy.deepcopy(entities)
                    info[0]["original_avail_actions"] = copy.deepcopy(available_actions)
                    entity_masks, entities, available_actions = env.reset(**data)
                remote.send(
                    (entity_masks, entities, reward, done, info, available_actions)
                )
            else:
        ...
        elif cmd == "get_entity_space":
            remote.send(env.entity_space)
        elif cmd == "render_vulnerability":
            fr = env.render_vulnerability(data)
            remote.send((fr))
        elif cmd == "get_num_agents":
            remote.send((env.n_agents))
        elif cmd == "get_num_entities":
            remote.send((env.n_entities))
        else:
            raise NotImplementedError
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def step_async(self, actions, **kwargs):
        for remote, actions_env in zip(self.remotes, actions):
            remote.send(("step", {"actions": actions_env, **kwargs}))
        self.waiting = True

    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        results = map(np.stack, zip(*results))
        return results
    ...
    def entity_space(self):
        if not getattr(self, "_entity_space", None):
            self.remotes[0].send(("get_entity_space", None))
            self._entity_space = self.remotes[0].recv()
        return self._entity_space

    @property
    def n_entities(self):
        if not getattr(self, "_n_entities", None):
            self.remotes[0].send(("get_num_entities", None))
            self._n_entities = self.remotes[0].recv()
...
class ShareDummyVecEnv(ShareVecEnv):
    def __init__(self, env_fns):
        assert False, "Outdated, don't use!"
